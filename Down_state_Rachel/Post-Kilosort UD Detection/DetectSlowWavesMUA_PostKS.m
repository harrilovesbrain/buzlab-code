...
function [SlowWavesMUA_PKS] = DetectSlowWavesMUA_PostKS(basePath,MUAchan,varargin)
...
%% Load Things
load([basePath filesep baseName '_imec0.spikes.cellinfo.mat'])
%load([basePath filesep baseName '_imec0.sessionInfo.mat'])
load([basePath filesep baseName '_imec0.session.mat'])

sr_wideband = session.extracellular.sr;
sr_lfp = session.extracellular.srLfp;
num_chs = length(MUAchan);
kk = length(session.epochs);
if isnumeric(session.general.duration)
    session_length_seconds = session.general.duration;
else
    session_length_seconds = (str2double(session.general.duration));
end
session_length_samples = ceil(session_length_seconds*sr_lfp);
...
for ii = 1:num_chs
    ...
    UD_units = find(spikes.maxWaveformCh1==MUAchan(ii));
